...
const OnboardingFlow: React.FC<OnboardingFlowProps> = ({
  ...
  const [phase, setPhase] = useState<Phase>(initialRole ? "welcome" : "role");
  ...
  const handleDecision = (flowKey: string) => {
    const data =
      role === "student"
        ? (studentData as any)[flowKey]
        : (jobSeekerData as any)[flowKey];
    triggerAnimation(() => {
      setActiveFlow(data);
      setPhase("flow");
      setStepIndex(0);
      ...
      >
        ...
        {phase === "decision" && role && (
          <>
            <div className="mb-6">
              <span
                className={`text-[10px] font-bold text-${accentColor}-600 tracking-widest uppercase mb-1 block`}
              >
                Step 01
              </span>
            ...
            <div className="space-y-3">
              {(role === "student"
                ...
                >
                  <div
                    className={`w-10 h-10 rounded-lg bg-slate-50 text-slate-600 flex items-center justify-center mr-4 group-hover:bg-${accentColor}-600 group-hover:text-white transition-colors`}
                  >
          ...
          <div className="flex flex-col h-full">
            <div className="mb-6">
              <div className="flex items-center justify-between mb-1.5">
                <span
                  className={`text-[10px] font-bold text-${accentColor}-600 tracking-widest uppercase`}
                >
                  Question 0{stepIndex + 1}
                </span>
                <span className="text-[10px] font-semibold text-slate-400 uppercase tracking-widest">
                  of 0{activeFlow.length}
                </span>
              </div>
            ...
            <div className="space-y-2.5 flex-grow">
              {activeFlow[stepIndex].type === "single" &&
                activeFlow[stepIndex].options.map((opt: any) => (
                  <OptionItem
                    key={opt.id}
                    icon={opt.icon}
                    label={opt.label}
                    ...
                    onClick={() =>
                      setAnswers((prev) => ({
                        ...prev,
                        [activeFlow[stepIndex].id]: opt.id,
                      }))
                    }
                  />
                ))}
                  ...
                  <div className="relative bg-white rounded-xl">
                    <input
                      ...
                      onChange={(e) =>
                        setAnswers((prev) => ({
                          ...prev,
                          [activeFlow[stepIndex].id]: e.target.value,
                        }))
                      }
        ...
        {phase === "success" && (
          <div className="text-center py-8">
            ...
            <button
              onClick={onComplete}
              className="w-full py-3.5 bg-slate-900 hover:bg-slate-800 text-white rounded-xl font-semibold text-sm shadow-lg mb-4 active:scale-95 transition-all"
            >
              Go to Dashboard
            </button>
            <button
...
const RoleButton: React.FC<{
  ...
  <button
    onClick={onClick}
    className={`w-full group flex items-center p-4 bg-white border border-slate-100 rounded-xl hover:border-${color}-300 hover:shadow-md transition-all text-left active:scale-95`}
  >
    <div
      className={`w-10 h-10 rounded-lg bg-${color}-50 text-${color}-600 flex items-center justify-center mr-4 group-hover:bg-${color}-600 group-hover:text-white transition-colors`}
    >
      <i className={`fa-solid ${icon} text-lg`}></i>
    </div>
    <div className="flex-1">
      <span className="block text-base font-bold text-slate-900 mb-0.5">
        {title}
      </span>
      <span className="text-xs text-slate-500 font-medium">{sub}</span>
    </div>
    <i
      className={`fa-solid fa-arrow-right text-slate-300 group-hover:text-${color}-600 group-hover:translate-x-1 transition-all`}
    ></i>
  </button>
);
...
const OptionItem: React.FC<{
  ...
  accent: string;
  onClick: () => void;
}> = ({ icon, label, selected, multi, accent, onClick }) => {
  ...
  return (
    ...
    >
      {icon && (
        <div
          className={`w-8 h-8 rounded bg-slate-50 text-slate-500 group-hover:bg-${accentClass}-50 group-hover:text-${accentClass}-600 flex items-center justify-center mr-3.5 transition-colors text-sm ${selected ? `bg-${accentClass}-100 text-${accentClass}-600` : ""}`}
        >
      ...
      {multi ? (
        <div
          className={`w-5 h-5 rounded border flex items-center justify-center text-white text-[10px] transition-all ${selected ? `bg-${accentClass}-600 border-${accentClass}-600` : "border-slate-300 bg-white"}`}
        >
          <i
            className={`fa-solid fa-check transition-opacity ${selected ? "opacity-100" : "opacity-0"}`}
          ></i>
        </div>
      ) : (
        <div
